...
export default class ChinaRegionWheelPicker extends BaseComponent {
  ...
  componentDidMount() {
    webRegionAPI().then((area) => {
      ...
      let selectedProvinceIndex=0, selectedCityIndex=0, selectedAreaIndex=0
      let selectedProvince = provinces.find((item, index) => {
        if(item.name === this.props.selectedProvince) {
            selectedProvinceIndex = index
          return true;
        }
          return false;
      });
      let selectedCity = citys.find((item, index) => {
        if(item.name === this.props.selectedCity) {
          selectedCityIndex = 0;
          return true;
        }
          return false;
      });
      let selectedArea = areas.find((item, index) => {
        if(item.name === this.props.selectedArea) {
          selectedAreaIndex = index;
          return true;
        }
        return false;
      });
      console.log('thisstate', provinces, citys, areas, selectedProvince, selectedCity, selectedArea)
      this.setState({
        provinces,
        citys,
        areas,
